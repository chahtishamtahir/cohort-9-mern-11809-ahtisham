 ...
 * Manages form state cleanly initialized from noteToEdit without cascading renders.
 */
const NoteEditorForm = ({ noteToEdit, onClose, onSave }) => {
  const [title, setTitle] = useState(noteToEdit?.title || '');
  const [content, setContent] = useState(noteToEdit?.content || '');
  const [category, setCategory] = useState(noteToEdit?.category || 'General');
  const [isPinned, setIsPinned] = useState(Boolean(noteToEdit?.is_pinned || noteToEdit?.isPinned));
...
export const NoteEditorModal = ({ isOpen, noteToEdit, onClose, onSave }) => {
  ...
  return (
    <div className="modal-overlay" style={{ zIndex: 1000 }}>
      <NoteEditorForm
        key={noteToEdit ? (noteToEdit.id || noteToEdit._id || 'edit') : 'new'}
